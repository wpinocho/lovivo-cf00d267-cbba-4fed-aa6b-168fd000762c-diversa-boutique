        ...
        <Card className="group overflow-hidden border-2 hover:border-accent transition-all duration-300 hover:shadow-xl">
          <CardContent className="p-4">
            <Link to={`/productos/${logic.product.slug}`} className="block">
              <div className="aspect-square bg-muted rounded-lg mb-4 overflow-hidden relative">
                ...
                <div className="absolute top-3 left-3 flex flex-col gap-2">
                  ...
                  {logic.product.featured && (
                    <span className="bg-accent text-accent-foreground text-xs px-2.5 py-1 rounded-full font-bold shadow-lg">
                      Destacado
                    </span>
                  )}
                  {!logic.inStock && (
                    <span className="bg-foreground text-background text-xs px-2.5 py-1 rounded-full font-bold shadow-lg">
                      Agotado
                    </span>
                  )}
                </div>
              ...
              <h3 className="font-semibold text-base mb-1 line-clamp-2 group-hover:text-accent transition-colors">
                {logic.product.title}
              </h3>
                      ...
                      {opt.values.filter(val => logic.isOptionValueAvailable(opt.name, val)).map((val) => {
                        ...
                        return (
                          <button
                            ...
                            type="button"
                            onClick={() => logic.handleOptionChange(opt.name, val)}
                            className={`border-2 rounded-md px-3 py-1.5 text-xs font-semibold transition-all ${
                              isSelected 
            ...
            <div className="flex items-center justify-between mt-4">
              ...
              <Button
                ...
                className="bg-foreground hover:bg-accent text-background font-semibold disabled:opacity-50 transition-all"
              >
